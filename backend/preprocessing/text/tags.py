"""Moved from state import data: By moving this inside the if __name__ == "__main__": block, Python won't try to load your massive dataset into memory when predict.py imports this file.

The if __name__ == "__main__": block: Anything under this line only runs when you execute app.py or run python tags.py directly. If you type from preprocessing.text.tags import get_tags in another file, it gracefully skips this bottom section.

Added joblib.dump: It now saves the mlb exactly as it was fit to your dataset."""
...
import pandas as pd
import joblib # Added for saving the MLB

root_dir = Path(__file__).resolve().parent.parent.parent
sys.path.insert(0, str(root_dir))

# ==========================================
# 1. REUSABLE FUNCTIONS (Safe to import)
# ==========================================
...
def get_tag_features(row):
    tags = [tag for tag in row['tags_list'] if tag.strip()] 
    if not tags: return 'none', 0.0

    clean_title = row['title'].encode("ascii", "ignore").decode() 
...
if __name__ == "__main__":
    # We only import state data if this file is run directly
    from state import data

    print("Processing tags for the dataset...")
    
    data['tags_list'] = data['tags'].apply(get_tags)
    data['tags_count'] = data['tags_list'].apply(len)
    data['main_tag'], data['tags_title_overlapRatio'] = zip(*data.apply(get_tag_features, axis=1)) 

    # Getting the binarizer for the tags that are in top 500 tags_list
    all_tags = [tag for tags in data['tags_list'] for tag in tags] 
    ...
    tag_binarized = mlb.fit_transform(data['tags_filtered']) 

    data['topTagsBinarized'] = list(tag_binarized)
    
    # Save the fitted MLB so predict.py can use it!
    joblib.dump(mlb, 'tags_binarizer.pkl')
    print("Saved tags_binarizer.pkl successfully!")
